...
function parseChangeItems(body: string): ChangeItem[] {
	...
	const appendToLastChild = (text: string) => {
		...
		if (targetIndex >= 0) {
			currentItem.children[targetIndex] =
				currentItem.children[targetIndex] + " " + text.trim();
		} else {
...
export function ChangelogNoticeboard() {
	const [lastSeenVersion, setLastSeenVersion] = useAtom(
		changelogLastSeenVersionAtom,
	);
	...
	const hasUnseenVersion =
		Boolean(latestVersion) && latestVersion !== lastSeenVersion;
	...
	const visibleEntries =
		showAllEntries || status !== "ready" ? entries : entries.slice(0, 1);
	const canToggleEntries = status === "ready" && entries.length > 1;
	const toggleLabel = showAllEntries
		...
		return segments.map((segment, index) => {
			...
			if (segment.type === "code") {
				return (
					<code
						key={key}
						className="rounded bg-muted/60 px-1 py-0.5 text-xs font-semibold text-muted-foreground"
					>
						{segment.content}
					</code>
				);
			...
			<Dialog open={open} onOpenChange={handleOpenChange}>
				...
				>
					<DialogHeader className="shrink-0">
						...
						<DialogDescription>
							Stay in the loop with the latest tweaks to the wiki.
						</DialogDescription>
					</DialogHeader>
					<div className="hm-scrollbar grow space-y-6 overflow-y-auto pr-2">
						...
						{status === "ready" && entries.length === 0 && (
							<p className="text-sm text-muted-foreground">
								There are no changelog entries yet. Check back later!
							</p>
						)}
												...
												{change.children.length > 0 ? (
													<ul className="mt-1 list-disc space-y-1 pl-4 text-muted-foreground">
														{change.children.map((child, childIndex) => (
															<li
																key={`${entry.version}-${index}-${childIndex}`}
															>
																{renderFormattedText(child)}
															</li>
														))}
													</ul>
												) : null}
					...
					<DialogFooter className="shrink-0 border-t border-border/60 pt-3">
						<Button variant="ghost" asChild>
							<a
								href="https://github.com/vitorfdl/game-inazuma-eleven/issues"
								target="_blank"
								rel="noreferrer"
							>
