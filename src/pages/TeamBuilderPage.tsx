...
export default function TeamBuilderPage() {
  ...
  const favoriteSet = useMemo(() => new Set(favoritePlayerIds), [
    favoritePlayerIds,
  ]);
  ...
  useEffect(() => {
    ...
    if (
      activeSlotId && !formation.slots.some((slot) => slot.id === activeSlotId)
    ) {
      setActiveSlotId(null);
      setDetailsOpen(false);
    }
  ...
  const handleFormationChange = (formationId: FormationDefinition["id"]) => {
    setTeamState((prev) => {
      const previousFormation = formationsMap.get(prev.formationId) ??
        FORMATIONS[0];
      ...
      >
        <div
          className={cn(
            ...
            !hasPlayer && "border border-dashed border-white/30 bg-black/20",
          )}
        >
          {player
            ? (
            ...
            <>
              <div className="pointer-events-none absolute inset-x-0 bottom-0 rounded-xs bg-black/80 text-center shadow-2xl backdrop-blur">
                <span className="m-0 block text-xs font-semibold uppercase  text-white/95">
                  {player
                    ? getSlotDisplayValue(player, displayMode)
                    : null}
                </span>
              </div>
              <span
                ...
                style={{
                  background: positionColor.gradient ??
                    `${positionColor.primary}22`,
...
function getSlotPositionStyle(slot: FormationSlot) {
  ...
  const left = COLUMN_STOPS[columnIndex] ??
    ((columnIndex / Math.max(FIELD_COLUMNS - 1, 1)) * 100);
  const top = ROW_STOPS[rowIndex] ??
...
function getSlotDisplayValue(player: PlayerRecord, mode: DisplayMode) {
  const fallback = player.nickname || player.name;
  if (mode === "nickname") {
    return fallback;
  }
  const statValue = player.power[mode];
...
function SlotDetailsPanel(
  { slot, assignment, onAssign, onClearSlot }: SlotDetailsPanelProps,
) {
              ...
              <div className="space-y-3">
                <div className="space-y-2">
                  {[
                    {
                      label: "Shoot AT",
                      value: player.power.shootAT,
                      icon: Target,
                    },
                    {
                      label: "Focus AT",
                      value: player.power.focusAT,
                      icon: Swords,
                    },
                    {
                      label: "Focus DF",
                      value: player.power.focusDF,
                      icon: Shield,
                    },
                    {
                      label: "Wall DF",
                      value: player.power.wallDF,
                      icon: BrickWall,
                    },
                    {
                      label: "Scramble AT",
                      value: player.power.scrambleAT,
                      icon: Target,
                    },
                    {
                      label: "Scramble DF",
                      value: player.power.scrambleDF,
                      icon: ShieldCheck,
                    },
